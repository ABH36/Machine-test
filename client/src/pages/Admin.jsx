...
const Admin = () => {
    ...
    const fetchAdminData = async () => {
        ...
        try {
            ...
            toast.error("Security Clearance Failed: Access Denied");
        } finally {
            setLoading(false);
        }
    };
    ...
    const overviewCards = [
        { label: 'Total Revenue', value: `$${stats?.totalRevenue}`, icon: <FaMoneyBillWave />, color: 'bg-emerald-600' },
        { label: 'Verified Users', value: stats?.users.length, icon: <FaUsers />, color: 'bg-blue-600' },
        { label: 'Total Inventory', value: stats?.products.length, icon: <FaBoxes />, color: 'bg-purple-600' },
        { label: 'Platform Sales', value: stats?.orders.length, icon: <FaShoppingCart />, color: 'bg-rose-600' },
    ];
            ...
            <div className="container mx-auto p-4 lg:p-10">
                ...
                <div className="flex justify-between items-center mb-12">
                    <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }}>
                        <h1 className="text-6xl font-black tracking-tighter italic flex items-center gap-4">
                    ...
                    {overviewCards.map((card, i) => (
                        ...
                        >
                            <div className="relative z-10">
                                ...
                                <h3 className="text-5xl font-black mt-2 tracking-tighter">{card.value}</h3>
                            </div>
                ...
                {/* --- Control Tabs --- */}
                <div className="flex gap-4 mb-10 bg-white/5 p-2 rounded-3xl w-fit border border-white/10 backdrop-blur-md">
                    {['overview', 'users', 'inventory', 'global-orders'].map(tab => (
                        <button 
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`px-10 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all ${activeTab === tab ? 'bg-rose-600 text-white shadow-[0px_10px_20px_rgba(225,29,72,0.4)]' : 'text-gray-500 hover:text-white'}`}
                        >
                            {tab.replace('-', ' ')}
                        </button>
                    ))}
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="bg-white/5 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">
                                        ...
                                        <th className="p-8 text-right">Security Protocol</th>
                                    </tr>
                                </thead>
                            ...
                            {stats?.products.map(p => (
                                <div key={p._id} className="bg-[#151921] p-8 rounded-[3rem] border border-white/5 flex gap-8 items-center group hover:border-blue-500/30 transition-all">
                                    <div className="relative">
                                        <img src={p.image} className="w-28 h-28 object-cover rounded-[1.5rem] shadow-2xl group-hover:scale-110 transition-transform duration-500" alt="" />
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <h4 className="text-2xl font-black uppercase tracking-tighter mt-1">{p.name}</h4>
                                        <div className="flex items-center gap-2 mt-4 text-gray-500">
                                            <FaStore className="text-xs" />
                                            <p className="text-[10px] font-bold uppercase tracking-widest">{p.vendor?.name || 'External Node'}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {stats?.orders.map(o => (
                                        <tr key={o._id} className="hover:bg-white/[0.03] transition-colors">
                                            <td className="p-8">
                                                <p className="font-mono text-xs text-blue-500">#{o._id.toUpperCase()}</p>
                                                <p className="text-[10px] font-black text-gray-500 mt-1 uppercase">{new Date(o.createdAt).toLocaleDateString()}</p>
                                            </td>
                                            <td className="p-8">
                                                <p className="font-black text-sm uppercase">{o.user?.name}</p>
                                                <p className="text-[10px] text-gray-500 font-mono">{o.user?.email}</p>
                                            </td>
                        ...
                        <motion.div key="overview" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="bg-gradient-to-br from-rose-600/20 to-transparent p-10 rounded-[3rem] border border-white/5">
                                ...
                                <p className="text-gray-400 leading-relaxed text-sm">
                                    Current platform health is optimal. No unauthorized breaches detected. 
                                    Average transaction speed is within baseline. 
                                    New node registrations (Vendors) have increased by 12% this session.
                                </p>
                            ...
                            <div className="bg-[#151921] p-10 rounded-[3rem] border border-white/5 flex items-center justify-center">
                                <div className="text-center">
                                    <FaGlobe className="text-rose-600 text-7xl mx-auto mb-6 animate-spin-slow" style={{ animationDuration: '10s' }} />
                                    <h4 className="text-xl font-black uppercase tracking-[0.3em]">Live Ecosystem</h4>
                                    <p className="text-gray-500 text-xs mt-2 font-bold uppercase">Monitoring Global Node Interactions</p>
                                </div>
                            </div>
                        </motion.div>
                    )}
